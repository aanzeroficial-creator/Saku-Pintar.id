document.addEventListener('DOMContentLoaded', async () => {
    ...
    function formatItemNameWithQty(name, qty) {
        ...
        // 1. Cek jika berakhiran satuan panjang/berat/volume (cm, mm, m, gr, g, kg, ml, l)
        const lengthWeightRegex = /(\d+)\s*(cm|mm|m|gr|g|kg|ml|l)$/i;
        if (lengthWeightRegex.test(lowerName)) {
            return `${trimmedName} (${qty} buah)`;
        }

        // 2. Cek jika berakhiran format kuantitas (angka + satuan, e.g. 1 pack, 1 pasang, 1 pcs)
        const unitRegex = /(\d+)\s*([a-zA-Z]+)$/;
        const match = trimmedName.match(unitRegex);
        if (match) {
            const num = parseInt(match[1]);
            const unit = match[2];
            const unitLower = unit.toLowerCase();

            // Satuan umum yang didukung untuk diganti angkanya
            const allowedUnits = ['pack', 'pasang', 'pcs', 'lembar', 'buah', 'kotak', 'botol', 'lusin', 'biji', 'pasang'];
            if (allowedUnits.includes(unitLower)) {
                const totalQty = num * qty;
                const lastIndex = trimmedName.lastIndexOf(match[0]);
                const baseName = trimmedName.substring(0, lastIndex).trim();
                return `${baseName} (${totalQty} ${unit})`;
            }
        }

        // 3. Kasus khusus barang berpasangan (kaus kaki, sepatu) yang belum ada satuannya
        if (lowerName.includes('kaus kaki') || lowerName.includes('sepatu')) {
            return `${trimmedName} (${qty} pasang)`;
        }

        // 4. Default: Jika tidak ada satuan, cukup tambahkan angka kuantitas dalam kurung
        return `${trimmedName} (${qty})`;
    }
    ...
    try {
        ...
        kunciJawaban = activeStory.kunciJawaban || [];


        const semuaAdaKunci = daftarBelanjaan.every(item =>
            kunciJawaban.some(k => item.nama.toLowerCase().trim().includes(k.barang.toLowerCase().trim()))
        );
        ...
        if (semuaAdaKunci && daftarBelanjaan.length > 0) {
            ...
            const evaluasiBarang = daftarBelanjaan.map(item => {
                const match = kunciJawaban.find(k => item.nama.toLowerCase().trim().includes(k.barang.toLowerCase().trim()));
                return {
                    barang: item.nama,
            ...
            if (jmlKebutuhan > 0 && jmlKeinginan === 0) {
                ...
                evaluasiKeuangan = `Pengelolaan uang kamu Kurang Bijak karena kamu membeli barang Keinginan sama banyaknya dengan Kebutuhan. Cobalah lebih memprioritaskan kebutuhan di misi berikutnya!`;
            } else {
...
Jawab WAJIB dalam format JSON murni tanpa markdown: { "evaluasiBarang": [ { "barang": "nama", "kategori": "Kebutuhan" / "Keinginan", "alasan": "alasan spesifik sesuai cerita/kunci jawaban, maks 15 kata" } ], "evaluasiKeuangan": "komentar singkat keputusan belanja diawali dengan 'Pengelolaan uang kamu ...' menggunakan kata ganti 'kamu', maks 2 kalimat." }`;
            ...
            daftarBelanjaan.forEach(item => {
                const existing = groupedDaftar.find(g => g.nama.toLowerCase().trim() === item.nama.toLowerCase().trim());
                if (existing) {
                    existing.qty = (existing.qty || 1) + 1;
                } else {
                    groupedDaftar.push({ ...item, qty: 1 });
                }
            });

            groupedDaftar.forEach(item => {
                const formattedName = formatItemNameWithQty(item.nama, item.qty);
                aiTabelBody.innerHTML += `
                    <tr style="background: #ECF0F1; border-bottom: 1px solid #BDC3C7;">
                        <td style="padding: 15px;"><strong>${formattedName}</strong></td>
            ...
            const evaluasiBarang = (rawParsed.evaluasiBarang || []).map(item => {
                const match = kunciJawaban.find(k => item.barang.toLowerCase().trim().includes(k.barang.toLowerCase().trim()));
                if (match) {
                    return {
                        barang: item.barang,
                        kategori: match.kategori,
        ...
        // Group by item name (case insensitive) to display 1 row per item with aggregated quantity
        const getActualQty = (itemName) => {
            const trimmed = itemName.toLowerCase().trim();
            let count = daftarBelanjaan.filter(b => b.nama.toLowerCase().trim() === trimmed).length;
            if (count > 0) return count;

            count = daftarBelanjaan.filter(b =>
                b.nama.toLowerCase().trim().includes(trimmed) ||
                trimmed.includes(b.nama.toLowerCase().trim())
            ).length;

            return count > 0 ? count : 1;
        };

        const groupedEvaluasi = [];
        daftarEvaluasi.forEach(item => {
            const trimmedName = item.barang.trim();
            const existing = groupedEvaluasi.find(g => g.barang.toLowerCase().trim() === trimmedName.toLowerCase());
            if (!existing) {
                const actualQty = getActualQty(trimmedName);
                groupedEvaluasi.push({ ...item, qty: actualQty });
            }
        });

        groupedEvaluasi.forEach(item => {
            const kelasKategori = item.kategori.toLowerCase() === 'kebutuhan' ? 'kategori-kebutuhan' : 'kategori-keinginan';
            const formattedName = formatItemNameWithQty(item.barang, item.qty);

            let labelKeputusan = '';
            if (kunciJawaban && kunciJawaban.length > 0) {
                const match = kunciJawaban.find(k =>
                    item.barang.toLowerCase().trim().includes(k.barang.toLowerCase().trim()) ||
                    k.barang.toLowerCase().trim().includes(item.barang.toLowerCase().trim())
                );

                if (match) {
                    labelKeputusan = `<span style="font-size: 0.8rem; color: #27AE60; font-weight: bold; display: block; margin-top: 3px;">✓ Keputusan Benar (Ada di Kunci)</span>`;
                } else {
                    labelKeputusan = `<span style="font-size: 0.8rem; color: #E74C3C; font-weight: bold; display: block; margin-top: 3px;">✗ Keputusan Salah (Tidak Ada di Kunci)</span>`;
                }
            } else {
                if (item.kategori.toLowerCase() === 'kebutuhan') {
                    labelKeputusan = `<span style="font-size: 0.8rem; color: #27AE60; font-weight: bold; display: block; margin-top: 3px;">✓ Keputusan Benar (Kebutuhan)</span>`;
                } else {
                    labelKeputusan = `<span style="font-size: 0.8rem; color: #E74C3C; font-weight: bold; display: block; margin-top: 3px;">✗ Keputusan Salah (Keinginan)</span>`;
                }
            }
    ...
    if (kunciJawaban && kunciJawaban.length > 0) {
        totalKebutuhan = kunciJawaban.length;

        if (parsedData && parsedData.evaluasiBarang) {
            const barangDibeliUnik = [...new Set(parsedData.evaluasiBarang.map(item => item.barang.toLowerCase().trim()))];
            const matchedKeys = new Set();

            // 1. Hitung benarCount (kunci jawaban guru yang berhasil dipenuhi)
            kunciJawaban.forEach((k, index) => {
                const isMatched = barangDibeliUnik.some(nama =>
                    nama.includes(k.barang.toLowerCase().trim()) ||
                    k.barang.toLowerCase().trim().includes(nama)
                );
                if (isMatched) {
                    matchedKeys.add(index);
                    benarCount++;
                }
            });

            // 2. Hitung salahCount (barang yang dibeli yang tidak cocok dengan kunci jawaban guru manapun)
            barangDibeliUnik.forEach(nama => {
                const isMatch = kunciJawaban.some(k =>
                    nama.includes(k.barang.toLowerCase().trim()) ||
                    k.barang.toLowerCase().trim().includes(nama)
                );
                if (!isMatch) {
        ...
        if (totalKebutuhan > 0) {
            nilai = Math.min(100, Math.round((benarCount / totalKebutuhan) * 100));
        } else {
            nilai = 100;
        }
    } else {
        ...
        if (parsedData && parsedData.evaluasiBarang) {
            let totalKebutuhanCount = 0;
            parsedData.evaluasiBarang.forEach(item => {
                if (item.kategori.toLowerCase() === 'kebutuhan') {
                    benarCount++;
                    totalKebutuhanCount++;
                } else {
            ...
            totalKebutuhan = totalKebutuhanCount;
            if (totalKebutuhan > 0) {
                nilai = Math.min(100, Math.round((benarCount / totalKebutuhan) * 100));
            } else {
                nilai = 100;
    ...
    Swal.fire({
        title: 'Skor',
        html: `
            <div style="font-family: 'Fredoka', 'Segoe UI', sans-serif; text-align: center; padding: 10px;">
                <p style="color: #7F8C8D; font-size: 1.1rem; margin-top: 0; margin-bottom: 20px;">
                    Analisis keputusan belanjamu telah selesai dilakukan.
                </p>
                <div style="background: #FAFDFB; border: 2px solid #2ECC71; border-radius: 15px; padding: 15px 20px; margin-bottom: 20px; box-shadow: 0 4px 10px rgba(46,204,113,0.05);">
                    <span style="font-size: 0.9rem; color: #7F8C8D; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px; display: block; margin-bottom: 5px;">
                        Keputusan Benar
                    </span>
                    <span style="font-size: 1.4rem; color: #27AE60; font-weight: 800;">
                        ${benarCount} dari ${totalKebutuhan} Barang
                    </span>
                </div>
                <div style="background: #EBF5FB; border: 2px solid #3498DB; border-radius: 15px; padding: 20px; box-shadow: inset 0 2px 4px rgba(0,0,0,0.02);">
                    <span style="font-size: 0.95rem; color: #5D6D7E; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px; display: block; margin-bottom: 8px;">
                        Skor
                    </span>
                    <span style="font-size: 4rem; font-weight: 900; color: #2980B9; line-height: 1; text-shadow: 0 4px 10px rgba(41,128,185,0.15);">
                        ${nilai}
                    </span>
                </div>
            </div>
        `,
        confirmButtonText: 'Lihat Detail Evaluasi',
        confirmButtonColor: '#3498DB',
        allowOutsideClick: false
    }).then(() => {

        const scoreContainer = document.getElementById('nilaiScoreContainer');
        const evalNilai = document.getElementById('evalNilai');
        const evalBenarCount = document.getElementById('evalBenarCount');

        if (scoreContainer && evalNilai) {
            evalNilai.textContent = nilai;
            if (evalBenarCount) evalBenarCount.textContent = `${benarCount} dari ${totalKebutuhan}`;

            scoreContainer.style.display = 'flex';
            evalNilai.animate([
                { transform: 'scale(1)', color: '#2980B9' },
                { transform: 'scale(1.3)', color: '#27AE60' },
                { transform: 'scale(1)', color: '#27AE60' }
            ], {
                duration: 800,
                easing: 'ease-in-out'
            });
