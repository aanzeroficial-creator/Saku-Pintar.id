document.addEventListener('DOMContentLoaded', async () => {
    ...
    function processNextCheckoutItem() {
        ...
        let currentItem = keranjangPilihan[currentCheckoutIndex];
        let currentTotal = currentItem.harga * currentItem.jumlah;
        ...
        // Cari apakah ada barang berikutnya yang bisa dibeli dengan sisa uang saku
        let foundAffordableIndex = -1;
        for (let i = currentCheckoutIndex + 1; i < keranjangPilihan.length; i++) {
            let item = keranjangPilihan[i];
            let total = item.harga * item.jumlah;
            if (uangSaku >= total) {
                foundAffordableIndex = i;
                break;
            }
        }

        if (foundAffordableIndex !== -1) {
            // Pindahkan barang yang tidak sanggup dibayar saat ini ke akhir antrean
            const unaffordableItem = keranjangPilihan.splice(currentCheckoutIndex, 1)[0];
            keranjangPilihan.push(unaffordableItem);
            
            // Rekursif untuk memproses barang berikutnya di antrean baru
            processNextCheckoutItem();
            return;
        }

        // Jika sama sekali tidak ada barang tersisa yang sanggup dibeli
        triggerOutofMoneyCountdown(currentItem.nama);
    }

    function triggerOutofMoneyCountdown(itemName) {
        let timerInterval;
        Swal.fire({
            title: 'Uang Saku Tidak Cukup! ⚠️',
            html: `Uang sakumu tidak cukup untuk membeli <b>${itemName}</b>.<br><br>Game akan selesai otomatis dalam <b>5</b> detik...`,
            timer: 5000,
            ...
            didOpen: () => {
                const b = Swal.getHtmlContainer().querySelector('b:last-of-type');
                let timeLeft = 5;
    ...
    function adjustScale() {
        const widthThreshold = 1200; // Desain lebar dasar game belanja
        const heightThreshold = 700; // Desain tinggi dasar game belanja
        const winWidth = window.innerWidth;
        const winHeight = window.innerHeight;

        const scaleX = winWidth / widthThreshold;
        const scaleY = winHeight / heightThreshold;
        let scale = Math.min(scaleX, scaleY);

        // Batasi skala agar tidak terlalu kecil atau terlalu raksasa di tablet/tab
        ...
        if (scale >= 1.0) {
            // PC / Fullscreen: Use zoom to stretch and fill the screen perfectly without cutting off buttons
            document.body.style.width = (100 / scale) + 'vw';
            document.body.style.height = (100 / scale) + 'vh';
        } else {
            // Mobile / Tablet: Reset dimensions to 100vw/100vh so that content stretches fully without shrinking
            document.body.style.width = '100vw';
            document.body.style.height = '100vh';
        }
            ...
            requestFS.call(docEl).then(() => {
                if (screen.orientation && screen.orientation.lock) {
                    screen.orientation.lock('landscape').catch(err => {
                        console.log("Kunci orientasi landscape gagal:", err);
                    });
                }
            }).catch(err => {
                console.log("Mulai layar penuh gagal:", err);
            });
        }
    }
    ...
    // Jalankan layar penuh pada klik pertama di halaman (kecuali jika mengklik tombol pause/kembali)
    ...
    function akhiriGame() {
        ...
        if (daftarBelanjaan.length === 0) {
            ...
            groupedDaftar.forEach(item => {
                totalBelanjaTable += item.harga * item.qty;
                ...
                tr.innerHTML = `
                    ...
                    <td class="evaluasi-item-name">${formattedName}</td>
                    <td class="evaluasi-item-price">${formatRupiah(item.harga * item.qty)}</td>
                `;
    ...
    btnSimpanEvaluasi.addEventListener('click', async () => {
        ...
        try {
                ...
                const catatanFormatted = `
                    <div style="font-size:1.2rem; line-height:1.5;">
                        <div style="background:#eef7f2; padding:12px 15px; border-radius:8px; margin-bottom:12px; border-left:5px solid #27AE60; box-shadow:0 2px 5px rgba(0,0,0,0.05);">
                            ...
                            <strong>Skor (Kunci Guru):</strong> <span style="font-weight:900; color:#27AE60; font-size:1.30rem;">${nilai} / 100</span> (Keputusan Benar: ${benarCount} dari ${totalKebutuhan})<br>
            ...
            Swal.fire({
                title: 'Laporan Belanja Terkirim!',
                html: 'Rencana belanjamu berhasil disimpan.<br>Silakan klik tombol di bawah untuk melihat evaluasi!',
                icon: 'success',
                showConfirmButton: true,
                confirmButtonText: 'Lanjut Evaluasi',
                confirmButtonColor: '#27AE60',
                allowOutsideClick: false,
                allowEscapeKey: false,
                allowEnterKey: true
            ...
            btnSimpanEvaluasi.textContent = 'Simpan';
        }
    });
