...
window.clearStudentResults = async function(category = null) {
    try {
        let snapshot;
        const batch = db.batch();

        if (category === 'kuis') {
            // Hapus semua kecuali Perencanaan Keuangan dan Pilahku
            snapshot = await resultsCol.get();
            snapshot.forEach((doc) => {
                const data = doc.data();
                if (data.aktivitas !== "Perencanaan Keuangan" && data.aktivitas !== "Pilahku (Mix & Match)") {
                    batch.delete(doc.ref);
                }
            });
        } else if (category === 'plan') {
            // Hapus Perencanaan Keuangan saja
            snapshot = await resultsCol.where('aktivitas', '==', 'Perencanaan Keuangan').get();
            snapshot.forEach((doc) => {
                batch.delete(doc.ref);
            });
        } else if (category === 'mix-match') {
            // Hapus Pilahku (Mix & Match) saja
            snapshot = await resultsCol.where('aktivitas', '==', 'Pilahku (Mix & Match)').get();
            snapshot.forEach((doc) => {
                batch.delete(doc.ref);
            });
        } else {
